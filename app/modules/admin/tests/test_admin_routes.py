...
def test_add_single_role_to_user(test_client):
    """Test adding a single role to a user."""
    # Login as admin
    test_client.post('/login', data={
        'email': 'admin@test.com',
        'password': 'admin123'
    ...
    response = test_client.post(
        f'/admin/users/{regular_user_id}/roles/{guest_role_id}'
    )
...
def test_remove_role_from_user(test_client):
    ...
    response = test_client.delete(
        f'/admin/users/{regular_user_id}/roles/{standard_role_id}'
    )
